...
class Module_SOLUCIONARE:
    ...
    def credentials_solucionare():
        """Recupera as credenciais de login de .env
    ...
    def run_processes(self, client, interval=30, confirm=False):
        ...
        if confirm:
            input("Confira os dados na página antes de proseguir.\n")
        ...
        if nothing_new == "Nenhum email enviado":
            data = f"Não houveram movimentações para os códigos {codes} no dia {datetime.today().strftime('%d/%m/%Y')}."
            print(data)

        else:
            try:
                # Aguarda a janela do relatório ficar disponível. Failsafe para aguardar fim do carregamento
